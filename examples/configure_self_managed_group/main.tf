...
resource "aws_eks_addon" "coredns" {
  ...
  depends_on = [
    null_resource.configure_self_managed_group,
  ]
}
...
resource "null_resource" "configure_self_managed_group" {
  ...
  provisioner "local-exec" {
    command = templatefile(module.eks_jumphost_instance.execute_script_jumphost_path, {
      script = <<-EOT
      cat <<EOF | HTTPS_PROXY=socks5://127.0.0.1:8443 kubectl apply -f -
      apiVersion: v1
      kind: ConfigMap
      metadata:
        name: aws-auth
      ...
      data:
        mapRoles: |
          - rolearn: ${module.eks.self_managed_node_groups.self_managed_node_group.iam_role_arn}
